...
Try {
	...
	[string]$appVendor = ''
	[string]$appName = 'Bonjour'
	[string]$appVersion = '3.0.0.10'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$exitCode = Execute-MSI -Path "$dirFiles\Bonjour64.msi" -Action Install -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'bonjour' -CloseAppsCountdown 60

		## Show Progress Message (with the default message)
		Show-InstallationProgress

		## <Perform Pre-Uninstallation tasks here>
		$exitCode = Execute-MSI -Path "$dirFiles\Bonjour64.msi" -Action Uninstall -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
